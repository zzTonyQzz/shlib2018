...
<template>
  <router-link
    ...
    :to="{path: `/index/pBookDetails?bid=${bookInfo.bib}`}"
    :class="{
    ...
    <div class="book-about">
      ...
      <ul class="other-info">
        <li class="lend-details">
          <b>外借日期/馆 :</b><span>{{bookInfo.borrowtime}}</span><span class="location-cn" v-if="bookInfo.borrowlocationcn">{{bookInfo.borrowlocationcn}}</span>
        </li>
        <li class="return-date">
          <b>归还日期/馆 :</b><span>{{bookInfo.duetime}}</span><span class="location-cn" v-if="bookInfo.duelocation">{{bookInfo.duelocation}}</span>
        </li>
      </ul>
    </div>
  </router-link>
